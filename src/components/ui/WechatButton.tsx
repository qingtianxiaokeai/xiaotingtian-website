...
export default function WechatButton({ variant = 'link' }: Props) {
  ...
  return (
    <>
      ...
      {/* 弹窗：CSS animation 替代 framer-motion AnimatePresence */}
      {open && (
        <>
          <div
            onClick={() => setOpen(false)}
            className="wechat-backdrop fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm"
          />
          <div className="fixed left-1/2 top-1/2 z-[101] -translate-x-1/2 -translate-y-1/2">
            <div
              ...
              aria-modal="true"
              aria-label="微信二维码"
              className="wechat-card relative w-72 rounded-2xl border border-[var(--border)] bg-[var(--color-bg-surface)] p-6 shadow-2xl"
            >
              {/* 关闭按钮 */}
              <button
                onClick={() => setOpen(false)}
